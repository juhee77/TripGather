...
- **[JaCoCo 기반 커버리지 관리]**: 로직을 담는 전 계층(`service`, `controller`, `security`, `exception`)에 두 가지 기준을 강제합니다 — **클래스당 라인 80% 이상**, 그리고 **전체 분기(branch) 80% 이상**. 이 검증(`jacocoTestCoverageVerification`)은 `check`/`build` 수명주기에 연결되어 기준 미달 시 빌드가 실패합니다. (2026-09-10 기준 라인 **97.4%** (1,630/1,674), 분기 **82.7%** (614/742), 테스트 465개 전부 통과)

| 계층 | 라인 커버리지 | 분기 커버리지 |
|:--|--:|--:|
| `controller` | 100% (269/269) | 78.4% (80/102) |
| `exception` | 100% (47/47) | 100% (4/4) |
| `security` (oauth 포함) | 96.6% (143/148) | 96.3% (52/54) |
| `service` (storage 포함) | 96.3% (1,025/1,064) | 82.8% (472/570) |
