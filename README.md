...
- **[JaCoCo 기반 커버리지 관리]**: 로직을 담는 전 계층(`service`, `controller`, `security`, `exception`)에 두 가지 기준을 강제합니다 — **클래스당 라인 80% 이상**, 그리고 **전체 분기(branch) 75% 이상**. 이 검증(`jacocoTestCoverageVerification`)은 `check`/`build` 수명주기에 연결되어 기준 미달 시 빌드가 실패합니다. (2026-09-08 기준 라인 **95.8%** (1,604/1,674), 분기 **77.4%** (574/742), 테스트 439개 전부 통과)
...
| `service` (storage 포함) | 93.9% (999/1,064) | 75.8% (432/570) |
