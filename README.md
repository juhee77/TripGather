...
- **[JaCoCo 기반 커버리지 관리]**: 서비스 계층과 인증/예외 처리 계층(`security.oauth`, `exception`)의 라인 커버리지를 **클래스당 80% 이상**으로 유지하도록 강제하며, 이 검증(`jacocoTestCoverageVerification`)은 `check`/`build` 수명주기에 연결되어 기준 미달 시 빌드가 실패합니다. (2026-09-02 기준 강제 대상 라인 커버리지 **94.8%** (1,254/1,323 라인), 테스트 338개 전부 통과)
- **[단계적 커버리지 확대]**: 회귀 시 영향 범위가 큰 지점부터 강제 범위를 넓혀 갑니다. 소셜 로그인 플로우(`security.oauth`)와 전역 예외 핸들러는 각각 0%·23.5%에서 **100%**로 끌어올려 게이트에 편입했습니다. 아직 미편입 구간은 `security` 최상위(`JwtAuthenticationFilter` 46.7%, `StompHandler` 6.2%, `SecurityService` 66.7%)와 `controller` 계층(50.2%)입니다.
