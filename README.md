...
- **[JaCoCo 기반 커버리지 관리]**: 로직을 담는 전 계층(`service`, `controller`, `security`, `exception`)의 라인 커버리지를 **클래스당 80% 이상**으로 유지하도록 강제하며, 이 검증(`jacocoTestCoverageVerification`)은 `check`/`build` 수명주기에 연결되어 기준 미달 시 빌드가 실패합니다. (2026-09-02 기준 강제 대상 57개 클래스 라인 커버리지 **95.5%** (1,599/1,674 라인), 테스트 422개 전부 통과)

| 계층 | 라인 커버리지 |
|:--|--:|
| `controller` | 99.6% (268/269) |
| `exception` | 100% (47/47) |
| `security` (oauth 포함) | 96.6% (143/148) |
| `service` (storage 포함) | 93.5% (995/1,064) |

  측정 대상에서 빠지는 것은 QueryDSL 생성 클래스, 부트스트랩 클래스, 그리고 자체 로직이 없는 `dto`/`domain`/`config`/`repository`뿐입니다.
