...
- **[JaCoCo 기반 커버리지 관리]**: 서비스 계층과 인증/예외 처리 계층(`security`, `exception`)의 라인 커버리지를 **클래스당 80% 이상**으로 유지하도록 강제하며, 이 검증(`jacocoTestCoverageVerification`)은 `check`/`build` 수명주기에 연결되어 기준 미달 시 빌드가 실패합니다. (2026-09-02 기준 강제 대상 37개 클래스 라인 커버리지 **94.7%** (1,331/1,405 라인), 테스트 356개 전부 통과)
- **[단계적 커버리지 확대]**: 회귀 시 영향 범위가 큰 지점부터 강제 범위를 넓혀 갑니다. 인증 경로 전체(소셜 로그인, JWT 필터, STOMP 연결 인증)와 전역 예외 핸들러를 0%~66% 구간에서 **전부 100%**로 끌어올려 게이트에 편입했습니다. 아직 미편입 구간은 `controller` 계층(50.2%)입니다.
