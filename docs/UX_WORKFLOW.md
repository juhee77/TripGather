...
| 순위 | 항목 | 이유 | 규모 | 상태 |
|:--:|:--|:--|:--|:--|
| 1 | 여행 피드 → 둘러보기 통합 | 가장 큰 혼란의 원인. 프론트만 고쳐도 됨 | 중 | ✅ 완료 |
| 2 | 탭 부제 추가 | 통합 전이라도 "남의 것/내 것"을 즉시 구분 가능 | 소 | ✅ 완료 |
| 3 | 정기편 도입 | 서비스 차별점. 도메인·마이그레이션·UI 모두 필요 | 대 | ✅ 완료 |
| 4 | 명칭 여행 테마화 | 몰입감 향상. 기능 이해도와 트레이드오프 확인 필요 | 소 | 보류 |

4번은 "게이트"처럼 은유가 앞서면 처음 온 사용자가 무엇을 하는 곳인지 알기 어려워
몰입감보다 손해일 수 있습니다. A/B 로 확인한 뒤 결정하는 편이 안전합니다.

### 정기편 구현 범위 (3번)

`Gathering` 에 `recurrenceRule`(NONE/WEEKLY), `recurrenceDayOfWeek`, `recurrenceUntil` 을
추가했습니다(`V14`). **회차는 테이블에 쌓지 않고 조회 시 계산합니다** — 미리 생성해 두면
"어디까지 만들어 둘 것인가", "규칙이 바뀌면 이미 만든 회차는 어떻게 할 것인가" 같은
상태 관리 문제가 따라오기 때문입니다.

남은 것: **스탬프를 회차별로 남기는 것**은 이번 범위에 넣지 않았습니다.
회차를 식별할 무언가(날짜 기준이든 별도 레코드든)가 필요해 설계가 한 겹 더 들어갑니다.
"이번 달 4회 참석" 같은 기록을 만들려면 이 작업이 선행되어야 합니다.
