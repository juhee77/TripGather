...
- [x] "내 여행에 추가" 버튼 클릭 시 정상적으로 여정이 복제되어 내 목록에 즉시 나타남. (필터링 버그 수정 완료)
- [x] 모임 생성 시 여정 연결이 원활하게 작동함.
- [x] 여행 피드에서 공개된 여정만 노출됨.

## 버그 수정 노트 (추가)
- **여정 조회 이슈**: '내 여행' 탭에서 공개 여정만 필터링하던 로직을 수정하여, 본인의 비공개 여정(복제본 등)도 즉시 확인할 수 있도록 개선했습니다.
- **실시간 리프레시**: 상세 페이지에서 돌아왔을 때 데이터가 즉시 갱신되도록 윈도우 포커스 리스너를 추가했습니다.
